...
trait AuthTrait
{
    ...
    public function mappingValidatorEdit($data) {
        ...
        if (!empty($data['email'])) {
            ...
            $validate['email'] = 'required|string|email|max:255|unique:"'.ShopCustomer::class.'",email, '.$data['id'].',id';
        }
        //Dont update id
        unset($data['id']);
    ...
    public function mappingValidator($data) {
        ...
        $validate = [
            'first_name' => 'required|string|max:100',
            'email' => 'required|string|email|max:255|unique:"'.ShopCustomer::class.'",email',
