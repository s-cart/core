    ...
    <div class="card card-primary card-outline card-outline-tabs">
      <div class="card-header p-0 border-bottom-0">
        <ul class="nav nav-tabs" id="custom-tabs-four-tab" role="tablist">
          <li class="nav-item">
            <a class="nav-link" href="{{ sc_route('admin_template.index') }}" >{{ trans('template.local') }}</a>
          </li>
          <li class="nav-item">
  ...
  @endphp
        ...
        <div class="block-pagination clearfix m-10">
          ...
          <ul class="pagination pagination-sm mr-3 float-right">
            ...
            <li class="page-item"><a class="page-link pjax-container" href="{{ sc_route('admin_template_online') }}?page={{ $dataApi['current_page'] - 1}}" rel="prev">«</a></li>
            @endif
            @for ($i = 1; $i < $dataApi['last_page']; $i++)
                ...
                @else
                <li class="page-item"><a class="page-link" href="{{ sc_route('admin_template_online') }}?page={{ $i }}">{{ $i }}</a></li>
                @endif
            @endfor
            @if ($dataApi['current_page'] < $dataApi['last_page'])
            <li class="page-item"><a class="page-link pjax-container" href="{{ sc_route('admin_template_online') }}?page={{ $dataApi['current_page'] + 1}}" rel="next">»</a></li>
            @endif
          </ul>
        </div>
...
<script type="text/javascript">
  function installTemplate(obj,key, path) {
      ...
      $.ajax({
        type: 'POST',
        dataType:'json',
        url: '{{ sc_route('admin_template_online.install') }}',
        data: {
          "_token": "{{ csrf_token() }}",
          "key":key,
          "path":path,
